...
- ChoiceStock 공개 HTML 수집은 전체 후보군에 적용하지 않고, 관심종목 또는 사용자가 선택한 후보 상세 보강에만 적용한다.
...
상세 수집은 최종 후보 또는 사용자가 선택한 종목에 한정한다.
...
ChoiceStock은 종목당 하루 1회 이하 원칙을 유지한다. Russell 1000 전체에 ChoiceStock을 적용하지 않는다.
...
- 최종 후보 상세 또는 관심종목 편입 후 하루 1회 캐시에만 사용한다.
